...
// Config path for reading envVars
...
// Get VPS scraper URL from environment or config.json (same as app.js)
function VPS_SCRAPER_URL() {
  if (process.env.LSTV_SCRAPER_URL) return process.env.LSTV_SCRAPER_URL;
  if (process.env.VPS_SCRAPER_URL) return process.env.VPS_SCRAPER_URL;
...
function VPS_SCRAPER_KEY() {
  ...
  if (process.env.VPS_SCRAPER_KEY) return process.env.VPS_SCRAPER_KEY;
  const cfg = loadConfig();
...
const SCRAPER_REGISTRY = {
  ...
  'vps-sofascore': {
    id: 'vps-sofascore',
    name: 'VPS SofaScore',
    description: 'SofaScore football fixtures + TV scraper (VPS)',
    vpsEndpoint: '/scrape/sofascore',
    healthEndpoint: '/health/sofascore',
    hasHealthCheck: true,
    // Fetch today's listings
    testParams: {},
    requiresVPS: true,
